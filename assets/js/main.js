...
var cubyX = 350, cubyY = 380;
var colisionCuby = false;

var enemigos = [
	{ "rect2" : {}, "tipo" : "enemy" }, { "rect2" : {}, "tipo" : "enemy"}, { "rect2" : {}, "tipo" : "enemy" }, { "rect2" : {}, "tipo" : "enemy" }, { "rect2" : {}, "tipo" : "cuby"}
];
	...
	if(!saltando){
		...
		if(indPlataforma == -1){
			...
			for(i in enemigos){
				...
				const maskX = Math.round(markX+70);
				const corX = Math.round(enemigos[i]["rect2"].x);

				
				//console.log(maskX +" - "+corX);

				if(maskY <= corY-20 && maskY >= corY-26 && maskX < corX+enemigos[i]["rect2"].width && markX > corX){
			...
			if(enemigos[indPlataforma]["tipo"] == "cuby"){
				colisionCuby = true;
				markY+=0.5;
				cubyY+= 0.5;
				enemigos[4]["rect2"].y = cubyY;
